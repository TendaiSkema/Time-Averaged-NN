...
class Neuron:
    def __init__(self, input_size, value=1, learning_rate=0.1):
        self.lr = learning_rate
        self.input_size = input_size
        self.value = value
        self.weights = np.random.uniform(-1, 1, input_size)  # Initialize weights uniformly in [-1, 1]
        self.threshold = np.random.uniform(-1, 1)  # Initialize threshold uniformly in [-1, 1]
        self.outputs = []
    ...
    def backward(self, grad_output, avg_input):
        ...
        self.threshold += self.lr * np.mean(d_grad_output)  # Update threshold with the mean of the gradient
...
class Layer:
    def __init__(self, input_size, output_size, value=1):
        self.neurons = [Neuron(input_size, value) for _ in range(output_size)]
        self.input_size = input_size
    ...
    def get_average_threshold(self):
        return np.mean([neuron.get_average_threshold() for neuron in self.neurons]) if self.neurons else 0
...
class SimpleNetwork:
    def __init__(self, input_size, output_size, hidden_size: list[int], iterations=300):
        ...
        for i in range(len(hidden_size)):
            ...
            self.hidden_layers.append(Layer(i_size, o_size, value=1))
        self.output_layer = Layer(hidden_size[-1], output_size, value=1)
    ...
    def get_all_average_thresholdes(self):
        all_thresholdes = []
        for layer in self.hidden_layers:
            all_thresholdes.append(layer.get_average_threshold())
        all_thresholdes.append(self.output_layer.get_average_threshold())
        return all_thresholdes
...
def training_step(model:SimpleNetwork, x, y):
    ...
    if DEBUG:
        print("Average activations:",np.around(model.get_all_average_activations(),3), 
            ...
            "Average thresholdes:", np.around(model.get_all_average_thresholdes(),3)
        )
